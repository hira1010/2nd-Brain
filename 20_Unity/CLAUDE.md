...
> このファイルは Claude Code がプロジェクト起動時に自動読み込みするエントリポイントです。
> 規約本体は別ファイル（Core_共通テンプレ.md）に集約しているため、本ファイルは短く保ちます。
> 検証用Unityプロジェクトに配置して使用してください。
...
- `Spec.md`（プロジェクトルート直下、CLAUDE.md と並べて配置）
...
オオバ環境の場合、`~/.claude/CLAUDE.md` にも全プロジェクト共通のルール（Unity標準Button禁止・Awake禁止・命名規則など）が定義されています。
