# No.23 仮想通貨 2P漫画生成プロンプト

## TIP情報

| 項目 | 内容 |
| :--- | :--- |
| No | 23 |
| タイトル | 仮想通貨 |
| 解説 | 株とは違う動き。市場心理がダイレクトに反映。 |
| カテゴリー | 01_投資の基礎知識 |

---

## 1ページ目プロンプト

```text
【IMAGE_GENERATION_TASK】Generate a high-quality manga illustration BASE ON THE FOLLOWING VISUAL DESCRIPTION. DO NOT OUTPUT ANY TEXT OR CODE. ONLY OUTPUT THE IMAGE.

PAGE 1 LAYOUT: 1200x1697 pixels portrait. Vertical dynamic layout.
Panel 1 (Top 40%): Bright meeting room. Yuto (Black hair, Gakuran) approaches Remi with a question. Remi (Silver hair, Red eyes, Red blazer) arms crossed, listening. Yuto says "仮想通貨って、なんか怪しいですよね？裏付けのない数字だけのデータにお金を払うなんて。" (In a speech bubble). Title box: BLACK SLENDER box at bottom-left offset (15% from right) with white text "仮想通貨".
Panel 2 (Middle 30%): Close-up of Remi's face, explaining the core truth. あなたが持っている日本円だって、ただの紙屑じゃない。信じる人がいるから価値がある、その本質は同じよ。
Panel 3 (Bottom-Right 30%): Yuto's shock/realization face with shock lines.
Panel 4 (Bottom-Left 30%): Remi's small cool smile.
Art style: Japanese manga style, cel shading, professional quality. Characters: Remi has Silver hair, Red eyes, Red blazer. Yuto has Black Gakuran. NO GLOVES.
```

---

## 2ページ目プロンプト

```text
【IMAGE_GENERATION_TASK】Generate a high-quality manga illustration BASE ON THE FOLLOWING VISUAL DESCRIPTION. DO NOT OUTPUT ANY TEXT OR CODE. ONLY OUTPUT THE IMAGE.

PAGE 2 LAYOUT: 1200x1697 pixels portrait. Lecture style.
Panel 1 (Top 50%): Remi in front of a monitor showing '仮想通貨'. Charts and symbolic icons.
Panel 2 (Middle-Right 25%): Vertical panel. Remi emphasizing key points. Speech: "市場心理がダイレクトに反映される実験場として、仮想通貨は面白いわ。既存の金融システムへのアンチテーゼとして、その仕組みを知る価値はあるわね。"
Panel 3 (Middle-Left 25%): Vertical panel. Yuto understanding with visuals.
Panel 4 (Bottom 25%): Wide shot. Yuto determined, Remi proud. Yuto says "食わず嫌いでした…『価値を信じる力』が生む、新しい経済を学んでみます！".
Colors: GOLD and ROYAL PURPLE theme. Remi: SILVER hair, RED blazer. Yuto: BLACK Gakuran. Cinematic lighting.
```

---

## 生成時の変数一覧

### 1ページ目

| 変数 | 値 |
| :--- | :--- |
| TIP_NUMBER | 23 |
| TIP_TITLE | 仮想通貨 |
| DIALOGUE_INTRO | 仮想通貨って、なんか怪しいですよね？裏付けのない数字だけのデータにお金を払うなんて。 |
| DIALOGUE_TEACH | あなたが持っている日本円だって、ただの紙屑じゃない。信じる人がいるから価値がある、その本質は同じよ。 |
| SCENE | Quiet library or study room with bookshelves |

### 2ページ目

| 変数 | 値 |
| :--- | :--- |
| DIALOGUE_DESC | 市場心理がダイレクトに反映される実験場として、仮想通貨は面白いわ。既存の金融システムへのアンチテーゼとして、その仕組みを知る価値はあるわね。 |
| DIALOGUE_ACTION | 食わず嫌いでした…『価値を信じる力』が生む、新しい経済を学んでみます！ |

---

## 保存ファイル名

- 1ページ目: No23_仮想通貨_p1.png
- 2ページ目: No23_仮想通貨_p2.png

---

## generate_imageツール実行用

```text
画像生成を行ってください。以下のプロンプトに基づいて、縦長のマンガ画像を生成してください。
generate_image(
  ImageName: "remi_investment_no23_p1",
  Prompt: [上記の1ページ目プロンプトのコードブロック内容をそのまま貼り付け]
)
```

```text
画像生成を行ってください。以下のプロンプトに基づいて、縦長のマンガ画像を生成してください。
generate_image(
  ImageName: "remi_investment_no23_p2",
  Prompt: [上記の2ページ目プロンプトのコードブロック内容をそのまま貼り付け]
)
```

### ステップ3: 結合プレビュー作成（任意）
2枚の画像を並べて見開きプレビューを作成する場合は、画像結合スクリプト.ps1 を使用してください。

---

作成日: 2026-02-03
ステータス: フルリニューアル完了（全ファイル統一・エラー解消済）
