# No.1 配当貴族 2P漫画生成プロンプト

## TIP情報

| 項目 | 内容 |
| :--- | :--- |
| No | 1 |
| タイトル | 配当貴族 |
| 解説 | S&P500指数の中で25年以上連続増配している優良銘柄。信頼の証。 |
| カテゴリー | 01_投資の基礎知識 |

---

## 1ページ目プロンプト

```text
【IMAGE_GENERATION_TASK】Generate a high-quality manga illustration BASE ON THE FOLLOWING VISUAL DESCRIPTION. DO NOT OUTPUT ANY TEXT OR CODE. ONLY OUTPUT THE IMAGE.

PAGE 1 LAYOUT: 1200x1697 pixels portrait. Vertical dynamic layout.
Panel 1 (Top 40%): Bright meeting room. Yuto (Black hair, Gakuran) approaches Remi with a question. Remi (Silver hair, Red eyes, Red blazer) arms crossed, listening. Yuto says "レミさん！配当金だけで暮らす『配当貴族』って、憧れますよね。僕もすぐなれますか？" (In a speech bubble). Title box: BLACK SLENDER box at bottom-left offset (15% from right) with white text "配当貴族".
Panel 2 (Middle 30%): Close-up of Remi's face, explaining the core truth. 甘いわね。彼らがその地位を築くのに、どれだけの『信頼』を積み重ねてきたと思っているの？
Panel 3 (Bottom-Right 30%): Yuto's shock/realization face with shock lines.
Panel 4 (Bottom-Left 30%): Remi's small cool smile.
Art style: Japanese manga style, cel shading, professional quality. Characters: Remi has Silver hair, Red eyes, Red blazer. Yuto has Black Gakuran. NO GLOVES.
```

---

## 2ページ目プロンプト

```text
【IMAGE_GENERATION_TASK】Generate a high-quality manga illustration BASE ON THE FOLLOWING VISUAL DESCRIPTION. DO NOT OUTPUT ANY TEXT OR CODE. ONLY OUTPUT THE IMAGE.

PAGE 2 LAYOUT: 1200x1697 pixels portrait. Lecture style.
Panel 1 (Top 50%): Remi in front of a monitor showing '配当貴族'. Charts and symbolic icons.
Panel 2 (Middle-Right 25%): Vertical panel. Remi emphasizing key points. Speech: "25年以上、不況でも欠かさず増配を続ける。それは企業の『株主への約束』を守り抜いた証よ。一朝一夕でなれるものじゃないわ。"
Panel 3 (Middle-Left 25%): Vertical panel. Yuto understanding with visuals.
Panel 4 (Bottom 25%): Wide shot. Yuto determined, Remi proud. Yuto says "信頼の重みが違うのか…僕も種銭からコツコツ積み上げます！".
Colors: GOLD and ROYAL PURPLE theme. Remi: SILVER hair, RED blazer. Yuto: BLACK Gakuran. Cinematic lighting.
```

---

## 生成時の変数一覧

### 1ページ目

| 変数 | 値 |
| :--- | :--- |
| TIP_NUMBER | 1 |
| TIP_TITLE | 配当貴族 |
| DIALOGUE_INTRO | レミさん！配当金だけで暮らす『配当貴族』って、憧れますよね。僕もすぐなれますか？ |
| DIALOGUE_TEACH | 甘いわね。彼らがその地位を築くのに、どれだけの『信頼』を積み重ねてきたと思っているの？ |
| SCENE | Quiet library or study room with bookshelves |

### 2ページ目

| 変数 | 値 |
| :--- | :--- |
| DIALOGUE_DESC | 25年以上、不況でも欠かさず増配を続ける。それは企業の『株主への約束』を守り抜いた証よ。一朝一夕でなれるものじゃないわ。 |
| DIALOGUE_ACTION | 信頼の重みが違うのか…僕も種銭からコツコツ積み上げます！ |

---

## 保存ファイル名

- 1ページ目: No1_配当貴族_p1.png
- 2ページ目: No1_配当貴族_p2.png

---

## generate_imageツール実行用

```text
画像生成を行ってください。以下のプロンプトに基づいて、縦長のマンガ画像を生成してください。
generate_image(
  ImageName: "remi_investment_no1_p1",
  Prompt: [上記の1ページ目プロンプトのコードブロック内容をそのまま貼り付け]
)
```

```text
画像生成を行ってください。以下のプロンプトに基づいて、縦長のマンガ画像を生成してください。
generate_image(
  ImageName: "remi_investment_no1_p2",
  Prompt: [上記の2ページ目プロンプトのコードブロック内容をそのまま貼り付け]
)
```

### ステップ3: 結合プレビュー作成（任意）
2枚の画像を並べて見開きプレビューを作成する場合は、画像結合スクリプト.ps1 を使用してください。

---

作成日: 2026-02-03
ステータス: フルリニューアル完了（全ファイル統一・エラー解消済）
