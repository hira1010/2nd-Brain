...
// TODO $gameTemp.setDestination(x, y);
// タッチで移動するときの移動先を決めている。タッチ移動だと必ずダッシュしているようなので、敵キャラクターの動きを合わせる必要がある。


// TODO Game_Event.prototype.checkEventTriggerTouch
// イベントがプレイヤーに接触した判定を与えている。敵から攻撃されるスクリプトはここをいじる。


// TODO ダッシュの禁止
// TODO 特定のボタンを押しながら方向キーで、自キャラの向きのみ変更
// TODO ミニマップ
...
(() => {
    ...
    Game_Player.prototype.update = function (sceneActive) {
        ...
        if (this._attackMotion && !this._moving) {
            console.log("Fturnは" + Fturn);
            console.log("Eturnは" + Eturn);
            this.attackMotion();
            this._moving = true;
        }
        if (!this.isMoving()) {
    ...
    Game_Event.prototype.selectRandomEnemyAction = function (enemy) {
        ...
        if (player.isNearTheEvent(enemy._event)) {
            if (enemy.isFacingPlayer()) {
                //console.log("enemy ATTACK1");
                return ATTACK;
            } else {
                enemy._event.turnTowardPlayer();
                //console.log("enemy ATTACK2");
                return ATTACK;
            }
        } else {
