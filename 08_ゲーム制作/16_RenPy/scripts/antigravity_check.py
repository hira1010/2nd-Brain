...
def run_logic_tests():
    ...
    # テストB: 部位クリック (Ear)
    namespace["click_action"]("ear")
    assert_test("Ear click increases intimacy", namespace["p_intimacy"] == 1.5)
    assert_test("Anger increases by click", namespace["p_anger"] == 1)

    # テストC: 胸クリック (Chest)
    current_p = namespace["p_pleasure"]
    namespace["click_action"]("chest")
    assert_test("Chest click increases pleasure", namespace["p_pleasure"] == current_p + 5.0)
    assert_test("Chest click shows heart layer", "heart_layer" in renpy.shown_screens)

    # テストD: お仕置き判定
    namespace["p_anger"] = 20
    namespace["click_action"]("ear")
    assert_test("Anger > 20 triggers punishment jump", renpy.jump_target == "punishment_start")
    ...
    namespace["click_action"]("ear")
    # inc_int = 1.5, mult = 1.5. -> Result should be 2.25
    assert_test("Ribbon (multiplier 1.5) works", namespace["p_intimacy"] == 2.25)
