...
# -----------------------------------------------------------------------------
# 2. Ren'Py ファイルの解析 (Minimal Parser)
# -----------------------------------------------------------------------------
def load_renpy_definitions(filepath, namespace):
    ...
    # define PARTS_CONFIG = { ... } を抽出 (複数行対応)
    parts_match = re.search(r"define\s+PARTS_CONFIG\s*=\s*(\{.*?\})\s*(\n\n|\n$)", content, re.DOTALL)
    if parts_match:
        try:
            # Pythonの辞書形式として評価
            parts_str = parts_match.group(1)
            # 文字列内の音声をモック名に変更するなどの処理が必要な場合はここで行う
            namespace["PARTS_CONFIG"] = eval(parts_str)
            print(f"✅ Loaded PARTS_CONFIG from {filepath}")
        except Exception as e:
            print(f"⚠️ Failed to parse PARTS_CONFIG: {e}")

    # default 変数を抽出 (簡易版)
    default_vars = re.findall(r"default\s+(\w+)\s*=\s*(.*)", content)
    for var_name, var_val in default_vars:
        try:
            # "True", "False", "0.0", "0" などを評価
            namespace[var_name] = eval(var_val.strip())
        except:
            # 複雑な式は文字列のままか無視
            pass

# -----------------------------------------------------------------------------
# 3. logic.rpy の読み込みと実行
# -----------------------------------------------------------------------------
def run_logic_tests():
    print("--- 🔄 HeroineAdv Engine Logic Test (Ver 8.0 Full Spec) ---")
    ...
    # definitions.rpy から設定を読み込む
    load_renpy_definitions("game/definitions.rpy", namespace)
