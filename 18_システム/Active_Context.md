...
1. **台詞の重複禁止**: 1つのコマ内だけでなく、1枚の画像内、あるいは1つのエピソード全体で同じ台詞や吹き出しを繰り返さない。
2. **句読点の排除**: 吹き出し内のセリフには「。」や「、」などの句読点を一切使用しない（感嘆符「！」や疑問符「？」は可）。
3. **字体の統一**: 全ページで標準的なマンガフォント（ウェイト）を維持する。視認性のために勝手に太字にしない。
4. **字つぶれ防止（視認性向上）**: 「搾取」などの画数が多い漢字が潰れないよう、画像生成プロンプトで `CRISP`, `SHARP`, `HIGH RESOLUTION TEXT` などの解像度重視のキーワードを必ず含める。
